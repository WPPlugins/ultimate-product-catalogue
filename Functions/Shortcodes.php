...
function Add_Product_Inquiry_Form($Product) {
	...
	if ($CF_7_Installed) {
		...
		$ProductString .= str_replace(array('%PRODUCT_NAME%', '%PRODUCT_ID%'), array($Product->Item_Name, $Product->Item_ID), do_shortcode('[contact-form-7 id="' . $UPCP_Contact_Form->ID . '" title="' . $UPCP_Contact_Form->post_title . '"]'));
		$ProductString .= "</div>";
	}
